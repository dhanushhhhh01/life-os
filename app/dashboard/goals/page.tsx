...
var defaultGoals = [
  { name: "Master FastAPI & LLMs", progress: 45, category: "Tech", deadline: "Jun 2026", color: "from-purple-500 to-pink-500", ring: "#a855f7" },
  ...
  { name: "Complete Master Thesis", progress: 10, category: "Academic", deadline: "Dec 2026", color: "from-blue-500 to-cyan-500", ring: "#22d3ee" },
  { name: "Read 24 Books This Year", progress: 25, category: "Learning", deadline: "Dec 2026", color: "from-indigo-500 to-purple-500", ring: "#8b5cf6" },
];
...
var ringColors = {
  ...
  "from-blue-500 to-cyan-500": "#22d3ee",
  "from-indigo-500 to-purple-500": "#8b5cf6",
  "from-cyan-500 to-blue-500": "#22d3ee",
};
...
export default function GoalsPage() {
  ...
  async function loadGoals(uid) {
    ...
    if (!result.error && result.data.length === 0) {
      ...
      if (!seed.error) setGoals(seed.data.map(function(g) { return Object.assign({}, g, { ring: ringColors[g.color] || "#8b5cf6" }); }));
    } else if (!result.error) {
      setGoals(result.data.map(function(g) { return Object.assign({}, g, { ring: ringColors[g.color] || "#8b5cf6" }); }));
    }
    setLoading(false);
  }
    ...
    <div className="p-6 lg:p-8 max-w-3xl mx-auto space-y-6 stagger-children">
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          className={"px-4 py-2.5 text-white text-sm font-semibold rounded-xl hover:opacity-90 transition-all shadow-lg flex items-center gap-2 " + (showAdd ? "bg-white/10" : "bg-gradient-to-r from-purple-600 to-cyan-600 shadow-purple-500/20")}
        >
      ...
      <div className="glass-card p-6 rounded-2xl border border-white/[0.06]">
        <div className="flex items-center gap-6">
          <div className="relative">
            <CircularProgress size={80} strokeWidth={6} progress={mounted ? avgProgress : 0} color="#8b5cf6" />
            <div className="absolute inset-0 flex items-center justify-center">
              <span className="text-xl font-black text-gradient-cyan font-display">{avgProgress}%</span>
            </div>
          </div>
      ...
      {/* Add Goal Form */}
      {showAdd && (
        <div className="glass-card p-6 rounded-2xl border border-purple-500/20 animate-slide-up">
          <h2 className="text-xs font-semibold text-purple-400 uppercase tracking-[0.15em] mb-4">New Goal</h2>
          <div className="space-y-3">
            ...
            <button onClick={addGoal} disabled={saving} className="w-full py-3 bg-gradient-to-r from-purple-600 to-cyan-600 text-white font-semibold rounded-xl hover:opacity-90 transition-all text-sm shadow-lg shadow-purple-500/20 disabled:opacity-50">
            ...
            <div key={goal.id} className="glass-card p-5 rounded-2xl border border-white/[0.06] hover:border-white/[0.12] transition-all group">
              <div className="flex items-center gap-4">
                ...
                <div className="flex gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity">
                  ...
                  <button
                    onClick={function() { updateProgress(goal.id, 10); }}
                    className="px-3 py-1.5 bg-purple-500/10 hover:bg-purple-500/20 text-purple-400 rounded-lg text-xs transition-all"
                  >+10</button>
                  <button
