...
export default function CoachPage() {
  ...
  return (
    <div className="flex flex-col h-full">
      {/* Header */}
      <div className="p-5 border-b border-white/[0.06] flex items-center gap-4 bg-[#080818]/60 backdrop-blur-xl">
        <div className="relative">
          <div className="w-11 h-11 rounded-xl bg-gradient-to-br from-purple-500 to-cyan-500 flex items-center justify-center text-white font-black text-lg shadow-lg shadow-purple-500/20 animate-pulse-glow">
            <Sparkles size={20} />
          </div>
      ...
      <div className="flex-1 overflow-y-auto p-5 space-y-4 scrollbar-hide">
        {messages.map(function(msg) {
          return (
            <div key={msg.id} className={"flex " + (msg.role === "user" ? "justify-end" : "justify-start") + " animate-slide-up"}>
              {msg.role === "dex" && (
                <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-purple-500/20 to-cyan-500/20 border border-purple-500/20 flex items-center justify-center text-purple-400 shrink-0 mr-2.5 mt-0.5">
                  <Sparkles size={14} />
                </div>
              )}
        ...
        {isTyping && (
          <div className="flex items-center gap-2.5 animate-slide-up">
            <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-purple-500/20 to-cyan-500/20 border border-purple-500/20 flex items-center justify-center text-purple-400 shrink-0">
              <Sparkles size={14} />
            </div>
        ...
        {quickPrompts.map(function(prompt) {
          return (
            <button
              ...
              className="text-xs px-3.5 py-2 rounded-full bg-white/[0.03] border border-white/[0.08] text-gray-500 hover:text-white hover:border-purple-500/30 hover:bg-purple-500/[0.05] transition-all"
            >
              {prompt}
            </button>
          );
      ...
      <div className="p-5 pt-0 border-t border-white/[0.06]">
        <div className="flex gap-3 mt-3">
          <button
            onClick={toggleVoice}
            className={"p-3 rounded-xl border transition-all " + (isListening ? "bg-red-500/20 border-red-500/40 text-red-400 animate-pulse" : "bg-white/[0.04] border-white/[0.08] text-gray-600 hover:text-purple-400 hover:border-purple-500/30")}
          >
            <Mic size={18} />
          </button>
          <input
          ...
          <button
            ...
            className="px-5 py-3 bg-gradient-to-r from-purple-600 to-cyan-600 text-white font-semibold rounded-xl hover:opacity-90 transition-all disabled:opacity-30 text-sm shadow-lg shadow-purple-500/20 flex items-center gap-2"
          >
            <Send size={15} />
